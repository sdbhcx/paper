...
 Code & Demo: dylanorange.github.io/projects/geal
I Dataset & Benchmark: huggingface.co/datasets/dylanorange/geal
...
large-scale pre-trained 2D models.
We employ a dual-
...
point clouds.
A granularity-adaptive fusion module and
...
Clean
Add Local
If you want to lift the [bag], at which point is your finger
most likely to carry the [bag]?
Add Global
Drop Local
Drop Global
Rotate
...
LASO
Ours
Figure 1. 3D affordance prediction under varied data noises.
...
arXiv:2412.09511v1 [cs.CV] 12 Dec 2024
...
scales and granularities.
The 2D-3D consistency align-
...
contexts [39, 44, 45, 50].
Building on this, some stud-
...
bridging the 3D-2D gap.
Treating the affordance score
...
h2D = ω2D(Q2D),
(4)
where h3D ∈RB×Ctxt×L and h2D ∈RB×V ×Ctxt×L, with L
...
An illustration of the Granularity-
...
sor f 2D
con ∈RB×V ×(m×C2D)×N I. Inspired by previous works
...
(a)
(b)
Figure 3. Illustration of the Granularity-Adaptive Fusion Mod-
...
granularity visual features in a similar manner.
How-
...
ˆy2D is obtained by multiplying h2D
enh with f 2D
enh, followed by a
sigmoid activation:
ˆy2D = sigmoid
...
enh
(9)
where ˆy2D ∈RN denotes affordance scores.
Training.
We employ a combination of Binary Cross-
...
ture extraction and affordance decoding.
Except for the
...
We conduct experiments on LASO [32] and
...
category setting.
On the PIAD dataset, GEAL achieves
...
0.390
0.102
pre-trained 2D models through Gaussian splatting.
The
...
the baselines.
This demonstrates that GEAL effectively
...
tion.
The robustness improvements are attributed to our
...
Ours
LASO
Identify the key points on the bottle that ensure a
...
the model.
Gaussian Splatting Configuration Tuning.
As shown
...
els.
Through a dual-branch architecture with Gaussian
...
• Jitter
– Description:
Adds Gaussian noise to perturb each
...
contain, open
Bottle •
contain, wrap grasp, open, grasp, pour
Bowl •
...
17 Affordance Types
Table 8.
Detailed statistics of the proposed LASO-C dataset,
...
sit, support, move
Bottle •
grasp, wrap grasp, open, contain, pour
Microwave •
open, contain
StorageFurniture •
open, contain
...
world deployment.
The dataset also emphasizes diverse
affordance scales and shapes, presenting significant chal-
lenges for perception models.
By addressing the se-
...
strate corresponding affordances.
For example, a point
...
min(Pi, Qi),
(13)
where the inputs are normalized such that P
i Pi
...
on both the PIAD and LASO datasets.
Among these,
...
XMF [1].
These baselines align image and point cloud
...
• IAGNet [70]:
Grounds 3D affordances by transfer-
...
Add Local
Add Global
Figure 7. Qualitative comparisons between GEAL and LASO [32] on the PIAD-C dataset, highlighting the superior robustness of our
method on corrupted data.
Ours
LASO
Describe your grasp method on the knife
...
improving autonomy and adaptability in industries.
...
arXiv preprint arXiv:2304.08492,
2023. 1
[8] Shengheng Deng, Xun Xu, Chaozheng Wu, Ke Chen, and
Kui Jia.
3d affordancenet: A benchmark for visual ob-
ject affordance understanding.
...
dance learning for robotic manipulation. In IEEE Interna-
tional Conference on Robotics and Automation, pages 5880–
5886, 2023. 1
...
In Conference on Robot Learning, pages 540–562. PMLR,
2023. 2
...
Seong Jae Hwang, and Hyunwoo J Kim.
Point cloud
augmentation with weighted local transformations.
IEEE/CVF International Conference on Computer Vision,
...
Liu.
Robo3d: Towards robust and reliable 3d perception
...
out-of-distribution depth estimation under corruptions.
...
hong Zhu, and Yuexian Zou.
G2l: Semantically aligned
...
30(1):79–82, 2005. 7, 13
[66] Shaoyuan Xie, Lingdong Kong, Wenwei Zhang, Jiawei Ren,
...
and Jianan Li.
Fusionrcnn: Lidar-camera fusion for two-
...
and Chun Yuan.
Convolution meets lora: Parameter effi-
